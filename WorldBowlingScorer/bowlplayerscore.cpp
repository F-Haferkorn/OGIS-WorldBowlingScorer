...
{   // add result QLabel with fFenderScorerame....
...
BowlPlayerScore::thrownCountOfPins(int pinsThrown){
    ...
    if(auto item = currentFrameScore() ; item )
    {
        ...
        if(auto prevItem= previousFrameScore(); prevItem&& prevItem->hasStrike())
        {
        ...
        else if(auto *previousItem = previousFrameScore() ; previousItem && previousItem->hasSpare() )
        {
...
{
    // update all: render and forward previuos score of all
    auto prevScore=0;
    ...
    for(int index=lastStrike; index>=firstStrikeInARow;  index--)
    {
        ...
        if( _item2 && _item2->hasStrike() )
        {
            if(lastStrike - index==0)           // Single Strike
            {
                ...
                /// for strike evaluation never use posisble 3rd throw in a frame (for last frame)
                /// add all available without 3rd throw of last frame
...
{
    auto *scoreFrame = currentFrameScore() ? currentFrameScore()->widgetFrame(): nullptr;

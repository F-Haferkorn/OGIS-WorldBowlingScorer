...
// if negative, cumulateover all throws
int
...
BowlFrameScore::thrownCountOfPins(int pinsThrown)
{
    ...
    if(!valueLabel)
        throw std::logic_error("unexpecetd exceeded range of rollLabel");
            ...
            )
    {
        ...
        widgetFrame()->setStyleSheet( "QWidget{ background-color : rgba( 255, 0, 0, 127);  }" );
    }
    else if( (pinsThrown == c_counOfAllPins) &&
             (  // last Score items allows multiple STRIKES
                ( (!m_isLastFrame) && noThrows()==1)       // strike only at first Throw
                ||  m_isLastFrame
             )  )
    {
        // rolled a STRIKE!!!
    ...
    {
        // only allow third throw if  on last frame if has Strike or hasSpare
        if(noThrows()>=c_maxCountOfRolls)
            return 0;
...
BowlFrameScore::isFrameClosed(bool useStrike )
{
    ...
    if(isLastFrame())
    {
        if(thwrownPins==3 )
           ...
           return thwrownPins == 2
                   && !( (useStrike &&hasStrike()) || hasSpare()) ;
    }
    else
